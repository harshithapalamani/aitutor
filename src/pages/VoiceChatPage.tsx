...
import { ChatMessage, RecordingState, VoiceSettings } from '../types';
...
const VoiceChatPage: React.FC<VoiceChatPageProps> = ({ 
  ...
  const speakMessage = async (text: string, useNativeLanguage = false) => {
    try {
      setIsPlaying(true);
      const language = useNativeLanguage ? nativeLanguage : voiceSettings.language;
      await speechService.speak(text, { ...voiceSettings, language });
    } catch (error) {
                  ...
                  {!message.isUser && (
                    <div className="flex items-center space-x-2 mt-2">
                      <button
                        ...
                        disabled={isPlaying}
                        className="p-1 hover:bg-gray-100 rounded-full transition-colors disabled:opacity-50 text-gray-600 hover:text-primary-600"
                      >
                        <Volume2 size={16} />
                      </button>
                      ...
                      {nativeLanguage !== 'en' && (
                        <button
                          ...
                          disabled={isPlaying}
                          className="p-1 hover:bg-gray-100 rounded-full transition-colors disabled:opacity-50 text-gray-600 hover:text-secondary-600"
